...
<head>
    ...
    <title>Emmanuel Adeboje | Feedback Info</title>
            ...
            <div class="theme-section">
                <div class="container">
                    <div class="row">
                        ...
                        <?php 
                        if($_GET['view']){
                        $feed = "SELECT * FROM em_feedback WHERE id =".$_GET['id'];
                        //echo $sel;
                        $feedr = mysql_query($feed) or die("An error occurred. Reason: ".mysql_error());
                        } 
                         while($ShowFeed = mysql_fetch_assoc($feedr)){ 
                        ?>
                        <div class="col-md-9">
                            <div class="blog-comment-section">
                                <h5 class="comment-title">Feedback Info</h5>
                                <div class="comment-form">
                                    <form action="" id="mike-comment-form" method="POST">
                                        <div class="form-group row">
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control" id="Name" value="<?php echo $ShowFeed["name"]; ?>" required>
                                        </div>
                                        <div class="col-sm-6">
                                            <input type="email" class="form-control" id="Email" value="<?php echo $ShowFeed["email"]; ?>" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control" id="Phone" value="<?php echo $ShowFeed["phone"]; ?>" required>
                                        </div>
                                        <div class="col-sm-6">
                                            <input type="text" class="form-control" id="Company" value="<?php echo $ShowFeed["subject"]; ?>">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <textarea class="form-control" name="your_message" rows="3"><?php echo $ShowFeed["message"]; ?></textarea>
                                    </div>
